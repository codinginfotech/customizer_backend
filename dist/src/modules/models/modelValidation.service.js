...
exports.validateModelBuffer = validateModelBuffer;
const shared_1 = require("@cpd/shared");
function parseGltfJson(buffer) {
    if (buffer.subarray(0, 4).toString('ascii') === 'glTF') {
        ...
        return { json: JSON.parse(jsonText), isBinary: true };
    }
    return { json: JSON.parse(buffer.toString('utf8')), isBinary: false };
}
...
function validateModelBuffer(buffer, fileName = 'model.glb') {
    ...
    try {
        ...
        valid = false;
        push('error', 'PARSE', `Not a valid GLB/GLTF file: ${err.message}`);
    }
    // Mesh names come from nodes when authors name nodes instead of meshes.
    const nodeNameByMesh = new Map();
    for (const node of json.nodes ?? []) {
        if (node.mesh !== undefined && node.name)
            nodeNameByMesh.set(node.mesh, node.name);
    }
    const meshNames = (json.meshes ?? []).map((m, i) => m.name || nodeNameByMesh.get(i) || `mesh_${i}`);
    const materialNames = (json.materials ?? []).map((m, i) => m.name || `material_${i}`);
    const zoneMeshes = meshNames.filter((n) => n.startsWith(shared_1.ZONE_MESH_PREFIX));
    let triangleCount = 0;
    let hasUVs = false;
    ...
    if (valid) {
        ...
        if (materialNames.length === 0)
            push('warn', 'NO_MATERIALS', 'No materials — presets cannot be mapped by name');
        else
            push('ok', 'MATERIALS', `${materialNames.length} materials`);
        if (!hasNormals)
            push('warn', 'NO_NORMALS', 'No normals — lighting will look faceted');
        if (!hasUVs)
            push('error', 'NO_UVS', 'No UV coordinates — designs cannot be applied');
        else if (missingUvMeshes > 0)
            push('warn', 'PARTIAL_UVS', `${missingUvMeshes} primitives have no UVs`);
        else
            push('ok', 'UVS', 'UV map detected on all primitives');
        if (zoneMeshes.length === 0) {
            push('warn', 'NO_ZONES', `No "${shared_1.ZONE_MESH_PREFIX}<areaKey>" meshes — bind print areas manually in the 3D studio`);
        }
        else {
            push('ok', 'ZONES', `Print zones: ${zoneMeshes.join(', ')}`);
        }
        if (triangleCount > 200_000)
            push('warn', 'HEAVY_GEOMETRY', `${(triangleCount / 1000).toFixed(0)}K triangles — consider decimation`);
        else if (triangleCount > 0)
            push('ok', 'GEOMETRY', `${(triangleCount / 1000).toFixed(1)}K triangles`);
        if (buffer.length > 15 * 1024 * 1024)
            push('warn', 'LARGE_FILE', `${(buffer.length / 1024 / 1024).toFixed(1)} MB file — will be slow on mobile`);
        if (maxTextureSize > 6 * 1024 * 1024)
            push('warn', 'LARGE_TEXTURE', 'Embedded texture exceeds ~6 MB');
        if (hasAnimations)
            push('warn', 'ANIMATIONS', 'Animations present — ignored by the product viewer');
        if (draco)
            push('ok', 'DRACO', 'Draco compression detected (decoder bundled in the viewer)');
    }
    // ---- score ----
    let score = 0;
    if (valid) {
        score = 40;
        if (hasUVs && missingUvMeshes === 0)
            score += 15;
        else if (hasUVs)
            score += 8;
        if (hasNormals)
        ...
        if (zoneMeshes.length > 0)
            score += 15;
        if (triangleCount > 500 && triangleCount <= 200_000)
            score += 5;
    ...
    return shared_1.modelValidationReportSchema.parse({
        valid,
        score,
        stats: {
...
//# sourceMappingURL=modelValidation.service.js.map